...
class ImportanceDDPM(DDPM):
    def __init__(self, network, T=100, beta_1=1e-4, beta_T=2e-2, k=10):
        ...
        self.t_num_samples = {t: 0 for t in range(T)}
    ...
    def update_prev_losses(self, t, new_loss):
        ...
        self.t_num_samples[t] = min(self.t_num_samples[t] + 1, 10)

        # Check if initialization is complete
        if self.initializing:
            self.initializing = not all(num == 10 for num in self.t_num_samples)
